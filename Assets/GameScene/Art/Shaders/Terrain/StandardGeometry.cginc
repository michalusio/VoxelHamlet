...
UNITY_DECLARE_TEX2DARRAY(_TextureMapArray);

// Vertex input attributes
struct VertexInput
{
	uint4 inData : POSITION;
};
...
Attributes Vertex(VertexInput input)
{
	...
	o.color = input.inData.w;
	o.position = mul(unity_ObjectToWorld, float4(input.inData.xyz, 1));
...
Varyings VertexOutput(float3 pos, half3 wnrm, half4 wtan, int col, float3 wpos)
{
	Varyings o;
	o.color = col;
#if defined(PASS_CUBE_SHADOWCASTER)
...
)
{
	...
	outStream.Append(VertexOutput(input[0].position, input[0].normal, input[0].tangent, input[0].color, input[0].worldPosition));
	outStream.Append(VertexOutput(input[1].position, input[1].normal, input[1].tangent, input[1].color, input[1].worldPosition));
	outStream.Append(VertexOutput(input[2].position, input[2].normal, input[2].tangent, input[2].color, input[2].worldPosition));
}
...
float mipmapLevel(in float2 uvs, in float size)
{
	uvs *= size;
...
)
{
	...
	// Sample textures
	half3 albedo;
	int blockIndex = input.color;

	float lod = mipmapLevel(uv, 2);
	albedo = UNITY_SAMPLE_TEX2DARRAY_LOD(_TextureMapArray, float3(uv, blockIndex), lod);
